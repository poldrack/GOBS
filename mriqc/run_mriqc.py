...
need to specify work dir so it doesn't use pwd
"""
...
workdir='/scratch/01329/poldrack/GOBS/workdir/mriqc'

t1data=layout.get(type='T1w',target='subject',return_type='id')

with open('mriqc_T1w.sh','w') as f:
    for subcode in t1data:
        cmd='singularity run %s %s %s participant --participant_label sub-%s -m T1w --nprocs 16 --mem_gb 64 --ants-nthreads 8 --ants-float -w %s'%(MRIQC,
                project_root,derivdir,subcode,workdir)
        f.write(cmd+'\n')
...
with open('mriqc_bold.sh','w') as f:
    for subcode in bolddata:
        cmd='singularity run %s %s %s participant --participant_label sub-%s -m bold --ica --nprocs 32 --mem_gb 64 --ants-nthreads 8 --ants-float -w %s'%(MRIQC,
                project_root,derivdir,subcode,workdir)
        f.write(cmd+'\n')
